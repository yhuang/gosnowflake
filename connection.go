...
func (sc *snowflakeConn) QueryContext(ctx context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	pattern := `[\w\-_]+`
	if match, err := regexp.MatchString(pattern, qid); !match {
		return nil, &SnowflakeError{
			Number : ErrQueryIDFormat,
			Message : fmt.Sprintf("Invalid QID %v", err),
			QueryID: qid}
	}
	...
	actualQuery := fmt.Sprintf("select * from table(result_scan('%s'))", qid)
	newCtx := context.WithValue(ctx, FetchResultByID, "")
	return sc.queryContextInternal(newCtx, actualQuery, args)
}
